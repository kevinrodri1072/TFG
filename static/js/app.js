...
var socket = io(location.protocol + '//' + location.hostname + ':5001');
        ...
        // ── XRFs ──
        // The XRF tools live on their own page (xrfs.html / xrfs.js). Here we
        // only reveal the "XRFs" toolbar button when this instance is a Twin;
        // clicking it opens /xrfs in a new tab (see index.html).
        function initXRFPanel() {
            fetch('/is_twin')
                .then(r => r.json())
                .then(data => {
                    if (data.is_twin) {
                        var btn = document.getElementById('btn-xrfs');
